  ...
  def custom_offer_create 
    ...
    if @custom.save
      if @custom.by_buyer?
        ...
        message_for_buyer = "<h4>I will #{@service.title}<span class='pull-right'>$ #{params['price']}</span></h4><a target='_blank' href='#{@service.primary_photo.image_url}'><img class='thumb' src='#{@service.primary_photo.image_url}'></a><p>#{link_to "Pending",show_custom_details_path(@service,@service.custom_packages.last),:target=>'_blank','class'=>'btn change-color' }</p>"
      else
