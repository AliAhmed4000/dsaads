class ServicesController < ApplicationController
  ...
  def custom_offer_create 
    ...
    if @custom.save
      if @custom.by_buyer?
        message_for_seller = "<h4>I can #{@service.title}<span class='pull-right'>$ #{params['price']}</span></h4><a target='_blank' href='#{@service.primary_photo.image_url}'><img class='thumb' src='#{@service.primary_photo.image_url}'></a><p id=#{@custom.id}>#{link_to "Accept",custom_offer_update_path(@custom,'approved'),'class'=>'mr-2 btn change-color','remote'=>'true'}#{link_to "Decline",custom_offer_update_path(@custom,'rejected'),'class'=>'mr-2 btn change-color','remote'=>'true'}</p>"
        ...
        message_for_buyer = "<h4>I can #{@service.title}<span class='pull-right'>$ #{params['price']}</span></h4><a target='_blank' href='#{@service.primary_photo.image_url}'><img class='thumb' src='#{@service.primary_photo.image_url}'></a><p id=#{@custom.id}>#{link_to "Accept",custom_offer_update_path(@custom,'approved'),'class'=>'mr-2 btn change-color','remote'=>'true'}#{link_to "Decline",custom_offer_update_path(@custom,'rejected'),'class'=>'mr-2 btn change-color','remote'=>'true' }</p>"
