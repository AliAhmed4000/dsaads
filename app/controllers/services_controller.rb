class ServicesController < ApplicationController
  ...
  def custom_offer_create 
    ...
    if @custom.save
      if @custom.by_buyer?
        message_for_seller = "<h4>I can #{@service.title}<span class='pull-right'>$ #{params['price']}</span></h4><a target='_blank' href='#{@service.primary_photo.image_url}'><img class='thumb' src='#{@service.primary_photo.image_url}'></a><p>#{link_to "Accept",custom_offer_update_path(@custom,'approved'),'class'=>'btn change-color','remote'=>'true'}#{link_to "Reject",custom_offer_update_path(@custom,'rejected'),'class'=>'btn change-color','remote'=>'true'}</p>"
        message_for_buyer = "<h4>I can #{@service.title}<span class='pull-right'>$ #{params['price']}</span></h4><a target='_blank' href='#{@service.primary_photo.image_url}'><img class='thumb' src='#{@service.primary_photo.image_url}'></a><p>#{link_to "Pending",show_custom_details_path(@service,@service.custom_packages.last),:target=>'_blank','class'=>'btn change-color' }</p>"
      else
        message_for_buyer = "<h4>I can #{@service.title}<span class='pull-right'>$ #{params['price']}</span></h4><a target='_blank' href='#{@service.primary_photo.image_url}'><img class='thumb' src='#{@service.primary_photo.image_url}'></a><p>#{link_to "Accept",custom_offer_update_path(@custom,'approved'),'class'=>'btn change-color','remote'=>'true'}#{link_to "Reject",custom_offer_update_path(@custom,'rejected'),'class'=>'btn change-color','remote'=>'true' }</p>"
        message_for_seller = "<h4>I can #{@service.title}<span class='pull-right'>$ #{params['price']}</span></h4><a target='_blank' href='#{@service.primary_photo.image_url}'><img class='thumb' src='#{@service.primary_photo.image_url}'></a><p>#{link_to "Pending",show_custom_details_path(@service,@service.custom_packages.last),:target=>'_blank','class'=>'btn change-color' }</p>"
      end
      chat = Chat.create!(
